...
def _who():
    ...
    The portfolio application puts the signed-in person's address on the
    request when it forwards one, and it builds that header itself from
    the session rather than copying anything the browser sent — so a
    browser cannot claim to be somebody else through that door.

    It is an attribution, not a permission: this service decides nothing
    on the strength of it, and anyone holding the basic-auth credentials
    reaches the same screens whatever it says. Reached directly, with no
    portal in front, it falls back to the basic-auth username.
    """
    v = (request.headers.get('X-Poel-User') or '').strip()
    if not v:
        a = request.authorization
        v = (a.username if a else '') or ''
    return v[:120]


def _may_see_history():
    """Whether to draw the history link.

    The portfolio application decides who may READ the history and refuses
    the request itself; this only decides whether a link is drawn, so that
    somebody who would be refused is not shown a door. Reached directly —
    no portal in front — the header is absent and the link stays, which is
    right: a direct visitor is holding this service's own credentials.
    """
    role = (request.headers.get('X-Poel-Role') or '').strip().lower()
    return role in ('', 'admin')


def _history_refusal():
    """A second lock on the history, from the inside.

    The portfolio application refuses these routes for anybody who is not
    an administrator, and that is what actually holds. This is the same
    rule stated again on this side, so the record of the firm's pipeline
    does not depend on a proxy rule staying correct. An absent header
    means nobody is proxying, and that is allowed through.
    """
    role = (request.headers.get('X-Poel-Role') or '').strip().lower()
    if role and role != 'admin':
        return ('The valuation history is kept to administrators. '
                'Running a valuation is not affected.'), 403
    return None
...
<div class="topbar">
 ...
 <span style="display:flex;gap:18px;align-items:center">
   {% if may_see_history %}<a class="tag" style="text-decoration:none" href="/valuations">Valuation history</a>{% endif %}
...
def _pdfs_to_case(docs):
    ...
    def _content_for(docs_f):
        ...
        # everything that varies per run lives HERE, after the documents; the
        # invariant guide/example/rules travel in `system` so they can be cached
        content.append({"type": "text", "text":
            f"Today's date is {dt.date.today().isoformat()} — use it as the valuation "
            "date 'vd' unless a document itself dictates one. Now extract the case JSON "
            "for the documents above, following the system instructions exactly."})
        return content
    # Static across every extraction run: marked for prompt caching so the
    # guide + example (~11K tokens) are billed at the cache-read rate after
    # the first call. It must stay byte-identical for a cache hit, so nothing
    # that varies per run (dates, filenames, documents) may appear in it.
    #
    # PROMPT_CACHE_TTL: '1h' (default) suits batch work - a desk pricing
    # several policies in an afternoon reads the block at a tenth of the price
    # all afternoon, and generation time counts against the shorter window.
    # '5m' for cheaper writes if runs are usually isolated; 'off' to disable.
    _ttl = (os.environ.get('PROMPT_CACHE_TTL') or '1h').strip().lower()
    _cache = {} if _ttl in ('off', 'none', '0') else {
        "cache_control": {"type": "ephemeral", "ttl": '5m' if _ttl == '5m' else '1h'}}
    _INSTR = ({"type": "text", **_cache, "text":
        "The documents in the user message all relate to ONE life-insurance policy. FIRST identify each "
        ...
        "mapping each filename (exactly as given in the user message) to its role. THEN extract ONE case "
        ...
        "- DEATH BENEFIT OPTION: read the death benefit option. If it is Option B / "
        "'Increasing' / Option 2 (death benefit = face + account value - the ledger's DB "
        "column GROWS with the account value), set top-level 'db_option': 'B', set 'face' "
        "to the TOTAL level coverage (basic face PLUS any term/ARTR rider coverage on the "
        "same insured - e.g. basic $10.5M + ARTR $10.5M = face 21000000), and transcribe "
        "the ledger 'ndb' as the PRINTED death benefit column (face + AV). Otherwise "
        "db_option is 'A'. Getting the TOTAL face right matters: term riders like ARTR "
        "are part of the death benefit.\n"
        "- CHARGES / CREDITS REPORT: if the illustration includes a charges breakdown "
        "(e.g. 'Summary of Policy Charges & Credits' with premium loads, admin charges, "
        "coverage charges and cost of insurance as SEPARATE columns), use it to populate "
        "the ledger charge fields: popc = the premium load as a FRACTION of premium "
        "(annual load $ / annual premium $), ppc = the MONTHLY per-policy admin charge "
        "($/12 of the annual admin), puc = the MONTHLY per-1000-of-face coverage charge "
        "(annual coverage charge / 12 / (face/1000)), set to 0 in years where the "
        "coverage charge has ended. Populating these lets the engine separate loads "
        "from mortality charges - CRITICAL for correct minimum premiums when the "
        "illustrated premium is far above the minimum.\n"
        ...
        "- Set 'vd' (valuation date) to the date stated at the end of the user "
        "message, not the illustration date, unless a document explicitly dictates "
        "a valuation date.\n"
        "- POLICY DATE: 'policy_date' must be the policy's true ISSUE/CONTRACT date - "
        "look for 'Issue Date' or 'Policy Date' on the contract, an Input Summary page, "
        "or a statement's policy-detail block. NEVER use the illustration's prepared "
        "date or a 'values as of' date as the policy_date: every anniversary, premium "
        "due date and policy-year boundary hangs off it, and an in-force illustration's "
        "'as of' date is usually months away from the real anniversary. Cross-check: "
        "the stated current policy year/month (e.g. 'Policy Year 12, Policy Month 3') "
        "must be consistent with the policy_date you chose. When only issue ages and "
        "the issue year are known, anchor the DOB to the true policy anniversary.\n"
        ...
        "- POLICY HEADER: the pricing engine does not need the carrier's name or the "
        "policy number, but a desk reading this does, so report them in a top-level key "
        "'_policy': {\"carrier\": the issuing company as printed (e.g. 'Lincoln National', "
        "'John Hancock'), \"policy_number\": the contract number exactly as printed, "
        "\"product_type\": one of UL, GUL, IUL, VUL, SUL, Whole Life, Term or Other, "
        "\"insured_last\": surname, \"insured_first\": forename(s), \"insured_state\": the "
        "two-letter state of issue or residence if printed}. Use null for anything not "
        "printed; do not infer a state from an address on a covering letter.\n"
        "- THE MEDICAL PICTURE: an LE report states what the estimate rests on, and that "
        "reasoning is what a reader has to weigh. Report it in a top-level key '_medical': "
        "{\"impairments\": a list of short strings, one per condition, each written as "
        "'System: finding' (e.g. 'Cardiovascular: coronary artery disease with five stents "
        "(2023)'); \"mitigating\": a list of strings for anything the underwriter treats as "
        "improving the outlook (a sustained weight loss, a resolved condition, good control "
        "on medication); \"underwriter_note\": one or two sentences in the underwriter's own "
        "terms on why the estimate is what it is; \"records_through\": the date the medical "
        "records run to, YYYY-MM-DD, if the report says}. Take these from the LE report, "
        "never from the illustration, and never invent a condition that is not named. Empty "
        "lists and nulls where the documents are silent - a blank is honest and a guess "
        "is not.\n"
        "- WRITING THE JSON: every string value must be a single line. Where a value "
        "needs a break, write the two characters backslash-n rather than a real line "
        "break - a raw newline inside a JSON string is not valid JSON and the whole "
        "reading is lost over it.\n"
    ...
    for scale in (1.0, 0.6, 0.35):
        ...
        try:
            msg = client.messages.create(
                ...
                # The case JSON now carries the policy header and the medical
                # picture as well as the ledger, and a long ledger was already
                # close to the old 8,000. Running out of room mid-JSON is not a
                # graceful failure, and unused headroom costs nothing.
                max_tokens=16000,
                system=[_INSTR],
    ...
    if getattr(msg, 'stop_reason', '') == 'max_tokens':
        raise ValueError(
            'The extraction ran out of room before it finished writing the case. '
            'Upload a shorter illustration - the ledger and summary pages are what '
            'matter - or split the documents and read them separately.')
    text = msg.content[0].text.strip()
    text = text[text.find('{'): text.rfind('}')+1]
    d = _loads_forgiving(text)
    ...
    policy_head = d.pop('_policy', None) or {}
    medical = d.pop('_medical', None) or {}
    ...
    defaults = dict(insured_name=None, illustration_name=None, payment_frequency='Quarterly',
                    ...
                    survivorship=False, insured2=None, db_option='A',
    ...
    return Case.from_json(tmp), notes, roles, runs, le_reports, policy_head, medical, prem_hist, prem_sum
...
@app.route('/')
def index():
    return render_template_string(PAGE, may_see_history=_may_see_history(), version=APP_VERSION, body='', pdf_ok=bool(os.environ.get('ANTHROPIC_API_KEY')))
...
def _json_escape_control(text):
    """Escape the raw control characters a model sometimes writes inside a
    JSON string.

    Free-text fields — an underwriter's assessment, the extraction notes —
    are prose, and prose invites a real line break. JSON forbids one inside
    a string, so `json.loads` stops with "Expecting ',' delimiter" at the
    line after it, which reads like a structural fault and is not one.

    Walks the text once, tracking whether it is inside a string and whether
    the last character was an escape, and rewrites only the control
    characters that appear inside one. Everything else is passed through
    untouched, so valid JSON survives this unchanged.
    """
...
def _loads_forgiving(text):
    """json.loads, with one repair attempt and an error somebody can act on.

    The model is asked for JSON and returns JSON; when it does not, the
    failure is almost always a line break inside a free-text value rather
    than anything structural. Repairing that is safe — valid input passes
    through the repair unchanged — and it turns a run that failed after two
    minutes of reading into one that succeeds.
    """
    try:
        return json.loads(text)
    except json.JSONDecodeError as first:
        try:
            d = json.loads(_json_escape_control(text))
            _log('extraction JSON needed repair (a raw line break inside a string)')
            return d
        except json.JSONDecodeError:
            snippet = ''
            try:
                lines = text.splitlines()
                if 0 < first.lineno <= len(lines):
                    snippet = ' near: ' + lines[first.lineno - 1].strip()[:110]
            except Exception:
                pass
            raise ValueError(
                'The reading came back in a shape this could not parse '
                f'({first.msg} at line {first.lineno}).{snippet} '
                'Try again — if it repeats, the documents are probably the '
                'kind that need reading by hand.')


def _log_cache(msg):
    """Record how the prompt cache behaved on an extraction call, so a low hit
    rate is visible in the logs rather than only on the API bill."""
    try:
        u = msg.usage
        w = getattr(u, 'cache_creation_input_tokens', 0) or 0
        r = getattr(u, 'cache_read_input_tokens', 0) or 0
        _log(f'extraction tokens: {u.input_tokens} fresh, {w} cache-write, '
             f'{r} cache-read ({"HIT" if r else "miss"}), {u.output_tokens} out')
    except Exception:
        pass
...
def value():
    # Read while the request is in hand, so the name is on the run even
    # though it is written a good deal further down.
    who = _who()
    try:
        ...
        if jsons:
            ...
            (case, notes_x, roles, runs, le_reports,
             _policy_head, _medical, prem_hist, prem_sum) = _pdfs_to_case(pdfs)
            ...
            if not case.funding:
                ...
                case.optimize_straddle = True
                notes += (' funding plan defaulted; the policy year straddling the valuation '
                          'date is optimized (the buyer controls payments from the VD - any '
                          'current-year installments already paid should be reflected via the '
                          'account-value override);')
        else:
        ...
        if res.get('db_option_b'):
            notes += (' DEATH BENEFIT OPTION B: the illustrated (overfunded) death benefit '
                      f'column was used only to calibrate charges; the buyer path is priced '
                      f'at minimum account value with the LEVEL total face of ${case.face:,.0f} '
                      '- the benefit an optimally-funded policy actually pays;')
        ...
        if res.get('db_option_b'):
            _c('Death benefit modeling', 'ok', 'engine',
               f'Option B (increasing): charges calibrated on the printed DB column; '
               f'buyer priced at the level total face ${case.face:,.0f}. Confirm the '
               'face includes ALL coverage layers (base + term/ARTR riders).')
        if res.get('dob_shifted_from'):
        ...
        return render_template_string(PAGE, may_see_history=_may_see_history(), version=APP_VERSION, body=body, has_result=True,
                                      pdf_ok=bool(os.environ.get('ANTHROPIC_API_KEY')))
    except Exception as e:
        _log('ERROR:', repr(e))
        traceback.print_exc()
        err = f'<div class="card err">Error: {e}\n\n{traceback.format_exc(limit=3)}</div>'
        return render_template_string(PAGE, may_see_history=_may_see_history(), version=APP_VERSION, body=err,
...
<div class="card">
 ...
 <div style="overflow-x:auto"><table id="histtable" style="min-width:900px">
  <tr>
   <td>Date</td><td>Ran by</td><td>Policy / Insured</td><td style="text-align:right">Face</td>
  ...
  {% for r in runs %}
  <tr class="hrow" data-k="{{(r.name ~ ' ' ~ r.insured ~ ' ' ~ (r.who or ''))|lower}}">
   <td style="white-space:nowrap">{{r.ts[:10]}}</td>
   <td style="white-space:nowrap"><span class="muted">{{r.who or '&mdash;'|safe}}</span></td>
...
def valuations():
    refused = _history_refusal()
    if refused: return refused
    embed = bool(request.args.get('embed'))
    body = render_template_string(HISTORY, runs=_history_runs(), embed=embed)
    return render_template_string(PAGE, may_see_history=_may_see_history(), version=APP_VERSION, body=body, history=True,
...
def valuations_del(job):
    ...
    if refused: return refused
    import shutil
    jd = _safe_job_dir(job)
    if os.path.isdir(jd): shutil.rmtree(jd, ignore_errors=True)
    from flask import redirect
    return redirect('/valuations')

def _safe_job_dir(job):
    """Resolve a job id to its directory, refusing anything that escapes
    JOBS_DIR (path-traversal defense; job ids are hex tokens we generate)."""
    jd = os.path.realpath(os.path.join(JOBS_DIR, os.path.basename(str(job))))
    root = os.path.realpath(JOBS_DIR)
    if not (jd == root or jd.startswith(root + os.sep)):
        abort(404)
    return jd
...
def download(job, kind):
    ...
    if not os.path.exists(p):
        ...
        return render_template_string(PAGE, may_see_history=_may_see_history(), version=APP_VERSION, body=msg, has_result=True,
...
def api_extract():
    """Read the documents and hand back what they say. Price nothing.

    The pricing screen has always done this on the way to a valuation; the
    portfolio application wants the same reading on its own, to fill in a
    new opportunity so nobody re-types a policy number off a PDF. So the
    step is exposed by itself: PDFs in, the facts out, and no job written
    to disk — the caller keeps what it wants and the documents are gone
    when this returns.

    Server to server. The portal holds this service's credentials and
    calls it directly; a browser never reaches here.
    """
    ufs = [uf for uf in request.files.getlist('files') if uf and uf.filename]
    uploads = [(uf.filename, uf.read()) for uf in ufs]
    if not uploads:
    ...
    if not os.environ.get('ANTHROPIC_API_KEY'):
        return {'error': 'Document reading is not configured on this service '
                         '(ANTHROPIC_API_KEY is unset).'}, 503

    pdfs = [(fn, b) for fn, b in uploads
            if os.path.splitext(fn)[1].lower() == '.pdf']
    if not pdfs:
        return {'error': 'Only PDFs can be read. Upload the illustration and any '
                         'life-expectancy reports as PDFs.'}, 400

    _log('extract: reading ' + ', '.join(f'{fn} ({len(b):,} bytes)' for fn, b in pdfs)
         + (f' for {_who()}' if _who() else ''))
    try:
        (case, notes, roles, runs, le_reports,
         policy_head, medical, prem_hist, prem_sum) = _pdfs_to_case(pdfs)
    except ValueError as e:
        return {'error': str(e)}, 422
    except Exception as e:
        _log(f'extract failed: {e}')
        return {'error': f'The documents could not be read ({e}).'}, 500

    # The ledger as a premium schedule: one row per policy year, the date it
    # falls due and what the illustration says is payable. This is the
    # schedule the one-pager prints, so it comes across whole rather than
    # as an average.
    schedule = []
    try:
        anchor_date = case.id_date or case.policy_date
        if anchor_date and case.ledger:
            first_py = min(case.ledger)
            for py in sorted(case.ledger):
                row = case.ledger[py] or {}
                due = dt.date(anchor_date.year + (py - first_py),
                              anchor_date.month, min(anchor_date.day, 28))
                schedule.append({'due_date': due.isoformat(),
                                 'amount': round(float(row.get('prem') or 0), 2)})
    except Exception as e:
        _log(f'extract: could not build a premium schedule ({e})')
        schedule = []

    first = case.ledger.get(min(case.ledger)) if case.ledger else {}
    return {
        'policy': {
            'carrier': policy_head.get('carrier'),
            'policy_number': policy_head.get('policy_number'),
            'product_type': policy_head.get('product_type'),
            'face_amount': case.face,
            'insured_last': policy_head.get('insured_last'),
            'insured_first': policy_head.get('insured_first'),
            'insured_state': policy_head.get('insured_state'),
            'dob': case.dob.isoformat() if case.dob else None,
            'gender': case.gender,
            'smoker': case.smoker,
            'account_value': case.av_at_id,
            'cash_surrender_value': (first or {}).get('csv'),
            'values_as_of': case.id_date.isoformat() if case.id_date else None,
            'annual_premium': round(float((first or {}).get('prem') or 0), 2) or None,
        },
        'medical': {
            'impairments': medical.get('impairments') or [],
            'mitigating': medical.get('mitigating') or [],
            'underwriter_note': medical.get('underwriter_note') or '',
            'records_through': medical.get('records_through'),
        },
        'le_reports': le_reports,
        'premium_schedule': schedule,
        'premium_history': prem_hist or [],
        'source_roles': roles,
        'illustration_runs': runs,
        'notes': notes,
        'read': [fn for fn, _ in pdfs],
    }
