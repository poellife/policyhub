...
const staff = await session('t@x.com','testtesttest');
...
await staff.goto(`${B}/#/policies`); await staff.waitForSelector('table.data tbody tr');
await staff.click('table.data tbody tr:first-child'); await staff.waitForSelector('.tabs');
...
check('cap table shows both holders', capText.includes('Harrison Family Trust') && capText.includes('M. Okonkwo'));
await staff.screenshot({path:`${S}/i3-policy-ownership.png`,fullPage:true});

console.log('\nINVESTOR VIEW');
const inv = await session('harrison@example.com','investorpass1');
...
await inv.goto(`${B}/#/policies`); await inv.waitForSelector('table.data tbody tr'); await inv.waitForTimeout(500);
...
await inv.goto(`${B}/#/settings`); await inv.waitForSelector('#pwForm'); await inv.waitForTimeout(500);
