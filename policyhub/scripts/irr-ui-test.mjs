...
import { xirr, fmtIrr } from '../public/irr.js';
...
check('a portfolio IRR tile is present', dashTiles.includes('Portfolio IRR'));
check('showing a percentage', /Portfolio IRR\s*[\d.]+%/.test(dashTiles.replace(/\s+/g, ' ')),
  (dashTiles.replace(/\s+/g, ' ').match(/Portfolio IRR [^·]{0,14}/) || [''])[0].trim());
...
check('the tab exists', tabNames.includes('Return / IRR'), tabNames.join(' / '));
...
const expected = xirr([...outflows, { date: PAID_ON, amount: CHEQUE }]);
const onScreen = (await p.locator('#calcIrr').textContent()).trim();
check('the calculator shows the exact rate as you type', onScreen === fmtIrr(expected),
  `${onScreen} expected ${fmtIrr(expected)}`);
...
check('the server computes exactly what the browser displayed',
  ...
  `server ${fmtIrr(saved.result.irr)} vs browser ${onScreen}`);
...
check('the headline now reads as realized', /Realized IRR/i.test(after));
...
check('the row carries the IRR', (await row.textContent()).includes(onScreen),
  (await row.textContent()).replace(/\s+/g, ' ').trim().slice(-60));
const regTiles = await p.locator('.kpi-row').textContent();
check('and the register shows a portfolio rate', /IRR/.test(regTiles));
...
if ((await ip.locator('.tabs').count()) > 0) {
  ...
  check('an investor can read the return', /IRR/.test(itext));
