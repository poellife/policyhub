...
const fails=[]; const errs=[]; const B='http://localhost:3000';
...
check('shows policy counts and totals', txt.includes('LCG2') && txt.includes('LCG3'));
...
await p.goto(`${B}/#/policies`); await p.waitForSelector('table.data tbody tr');
...
const funds = await (await p.request.get(`${B}/api/funds`)).json();
...
await p.click('dialog button[type=submit]'); await p.waitForTimeout(1400);
const cleared = await (await p.request.get(`${B}/api/policies?search=`)).json();
const target = cleared.find(x=>x.policy_number);
check('owner can be cleared back to none',
  (await (await p.request.get(`${B}/api/funds`)).json()).find(f=>f.code==='INLINE1').policy_count===0);
...
for (const code of ['TESTCO','INLINE1']) {
  const f = (await (await p.request.get(`${B}/api/funds`)).json()).find(x=>x.code===code);
  if (f) await p.request.fetch(`${B}/api/funds/${f.id}`,{method:'DELETE'});
}
